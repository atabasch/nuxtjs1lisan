...
router.post('/packets', (request, response)=>{
  ...
  db.query(sql, (err, res, fields) => {
    ...
    response.status(200).json({items:res})
  });
});
    ...
    if(err){      response.status(404).json( {err} );     }else{
      ...
      db.query(sqlWord, (err, result, fields) => {
        if(err){      response.status(404).json( {err} );     }else{
          ...
          response.status(200).json( {package:resultPackage[0], words:result} );
        } //if(err)
      });
    ...
    if(err){      response.status(404).json( {err} );     }else{
      ...
      db.query(sqlWord, (err, result, fields) => {
        if(err){      response.status(404).json( {err} );     }else{
          ...
          response.status(200).json( {category:category[0], words:result} );
        } //if(err)
      });
    ...
    if(err){      response.status(404).json( {err} );     }else{
      ...
      db.query(sqlWord, (err, result, fields) => {
        if(err){      response.status(404).json( {err} );     }else{
          ...
          response.status(200).json( {type:type[0], words:result} );
        } //if(err)
      });
...
router.post('/:start/:limit', (request, response)=>{
  ...
  db.query(sql, (err, res, fields)=>{response.status(200).json({ words:res }); });
});
