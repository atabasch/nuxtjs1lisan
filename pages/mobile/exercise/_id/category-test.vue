...
export default {
  ...
  asyncData(context){
    ...
    let requestUrl = `/exercise/test?taxonomy=category&id=${id}&limit=15`;
    return context.store.dispatch("words/fillWords", {url:requestUrl});
  }, //asyncData

  created() {
    this.$store.commit("setHeaderBar", {title:this.$store.getters["words/getCategory"].tax_name, prevUrl: '/mobile/words/packets'});
  },
